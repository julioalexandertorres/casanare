    ...
    return when.all(promises, function (results) {
      ...
      for (let resultIndex = 0; resultIndex < results.length; ++resultIndex) {
        ...
        if (defined(result) && result.length > 0) {
          if(result[0].properties.panoramas){
            infoPano(result[0].properties);
          }
          else{
          for (
            let featureIndex = 0;
...
function infoPano(data360){
  var foto = data360.panoramas;
  var dataStreet = data360;
  streetView360(dataStreet);
  document.getElementById("container3").style.display = 'block'; 
  var closeButton = document.querySelector('.cesium-infoBox-close');
  if (closeButton) {
      closeButton.click();
  }
  document.getElementById('container3').innerHTML='';
      PSV = new PhotoSphereViewer({
      panorama: 'https://storage.googleapis.com/panosbuenaventura/Imagenes_final/' + foto + '.jpg',
